...
4. В интерфейсе открыть вкладку `Настройки` и показать профиль водителя: имя, телефон, email, количество автомобилей и историю диагностик.

5. Открыть вкладку `Авто`, добавить или показать уже созданный автомобиль.

6. Открыть вкладку `Диагностика`, выбрать автомобиль и нажать `Демо-скан`. Демо создаёт сессию с кодами `P0301` и `P0118`.

7. В карточке диагностики показать:

- человекочитаемый уровень риска: `Высокая` / `Критическая`
- прогноз риска на будущее
- рекомендации водителю
- кнопку `Найти ближайшее СТО`
- кнопку `Скачать PDF-отчет`

8. В истории диагностик показать фильтры по автомобилю, уровню риска, DTC-коду и диапазону дат.

9. Перейти по кнопке `Найти ближайшее СТО` в справочник, открыть вкладку `СТО`, нажать `Использовать моё местоположение` или показать поиск по Алматы/Астане. На карте отображаются СТО с координатами и специализацией.

10. Вкладка `Справочник` позволяет показать расшифровку DTC-кода, например `P0118`, на русском языке.

Для продакшен-карт можно передать свой tile provider:

```bash
flutter run -d chrome \
  --dart-define=MAP_TILE_URL="https://your-tile-provider/{z}/{x}/{y}.png" \
  --dart-define=MAP_TILE_ATTRIBUTION="© Your Provider"
```
